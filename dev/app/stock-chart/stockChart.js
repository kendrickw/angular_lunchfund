...
(function () {
    ...
    app.controller('StockChartController', [ 'global', 'stockstat', '$scope', '$filter', '$http', 'message', function (global, stockstat, $scope, $filter, $http, message) {
        ...
        // Function to refresh stock event list
        function refreshStockevent() {
            $http.get('/db/stockevent', {
                params: {
        ...
        // Function to add stock event
        function addToStockevent() {
            me.stockform.symbol = $scope.stock.symbol;
            me.stockform.buy = me.stockform.type === 'BUY' ? 1 : 0;
            $http.post('db/stockevent', me.stockform).success(function (data) {
                message.showInfo('STOCKEVENT_ADD_SUCCESS', [ me.stockform.symbol ]);
            }).error(function (error, status, headers, config) {
                message.showError('GENERIC', [JSON.stringify(error)]);
            });
        }
        ...
        angular.extend(me, {
            ...
            addToStockevent: addToStockevent
        });
